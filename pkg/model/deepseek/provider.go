...
func New(apiKey, baseURL string) *Provider {
	return NewWithName("deepseek", apiKey, baseURL, 128000)
}

// NewWithName 创建自定义名称的 DeepSeek Provider
...
func (p *Provider) Chat(ctx context.Context, req *model.ChatRequest) (*types.ChatResponse, error) {
	...
	// DeepSeek 额外的 usage 字段
	if apiResp.Usage.ThinkingTokens > 0 {
		usage.ReasoningTokens = apiResp.Usage.ThinkingTokens
	}
	if apiResp.Usage.CacheReadTokens > 0 {
		usage.CacheReadTokens = apiResp.Usage.CacheReadTokens
	}
	if apiResp.Usage.CacheWriteTokens > 0 {
		usage.CacheWriteTokens = apiResp.Usage.CacheWriteTokens
	}
...
func (p *Provider) ChatStream(ctx context.Context, req *model.ChatRequest, cb model.StreamCallback) (*types.ChatResponse, error) {
	...
	for scanner.Scan() {
		...
		if chunk.Usage != nil {
			...
			if chunk.Usage.ThinkingTokens > 0 {
				usage.ReasoningTokens = chunk.Usage.ThinkingTokens
			}
...
func (p *Provider) buildRequestBody(req *model.ChatRequest, stream bool) map[string]any {
	...
	for _, m := range req.Messages {
		...
		// DeepSeek 思考模式：将 reasoning_content 传回 API
		if m.Reasoning != "" {
